...
(function () {
    try {
        ...
        var links = document.querySelectorAll('.nav-menu a, .navbar-nav a');
        links.forEach(function (a) {
            var href = a.getAttribute('href');
            if (!href) return;
            // Normalize: if href ends with / treat as index.html
...
// Sticky behaviour for `.site-header` (works on all pages)
(function () {
    try {
        var header = document.querySelector('.site-header');
        var spacer = document.getElementById('header-spacer');
        if (!header) return;
        var triggerHeight = header.offsetTop;
        ...
        console.error('header sticky error', e);
    }
})();
